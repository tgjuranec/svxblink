    ...
    rem_audio_fifo(0), ptt_valve(0), tx_audio_splitter(0), vox(0), dns(0)
{
  ...
  Async::IpAddress ip;
  ip.setIpFromString(remote_ip.toStdString());
  station.setIp(ip);
  station.setCallsign(callsign.toStdString());
  station.setDescription("that is a description");
  station.setTime("NOW");
...
void ComDialog::onStationListUpdated(void)
{
  ...
  StationData st;
  Async::IpAddress ip;
  ip.setIpFromString("192.168.1.13");
  st.setIp(ip);
  st.setCallsign("9A5ATG");
  st.setDescription("that is a description");
  st.setTime("2016-01-22 25:56");
  station = &st;
  //station = 0;
  updateStationData(station);
  if (con == 0)
